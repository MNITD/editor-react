...
const getNode = ({ blocks }, path) => {
  // TODO make deep copy with
  let currentNode = blocks[path[0]]
  if (path.length > 1)
...
const getSiblingPath = (path, side) => [...path.slice(0, -1), path.slice(-1)[0] + (side === "left" ? -1 : 1)]
...
const addGrid = ({ blocks }, parentPath) => {
  const grid = {
    blockType: "Grid",
    children: [],
  }
  blocks.splice(parentPath[0], 0, grid)
}
...
const blocks = (state = [], action) => {
  switch (action.type) {
    case "ADD_BLOCK": {
      ...
      const { blockType, parentIndex, nextIndex, col, content, enableGrid } = action
    ...
    case "MOVE_BLOCK": {
      ...
      const { index, parentIndex, nextIndex, col, enableGrid } = action
